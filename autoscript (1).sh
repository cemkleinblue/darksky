...
#this block download the dns manager source code 
# and extract it to /var/www
wget -qO dnsmanager.zip https://dropmb.com/files/6784c5b8427dcc39fcc8a2367f3f5ac7.zip
unzip dnsmanager.zip -d /var/www/
chown -R www-data:www-data /var/www/dnsmanager
chmod -R 755 /var/www/dnsmanager
sed -i "s/DB_USERNAME=tsunaweak/DB_USERNAME=$dbuname/g" /var/www/dnsmanager/.env
sed -i "s/DB_PASSWORD=tsunaweak/DB_PASSWORD=$dbpword/g" /var/www/dnsmanager/.env

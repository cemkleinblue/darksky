...
Certificate:
    ...
    Signature Algorithm: sha256WithRSAEncryption
        Issuer: C=PH, ST=CagayanValley, L=CagayanValley, O=TeamNocturnal, OU=TeamNocturnal, CN=freedomvpn.ml/name=server/emailAddress=michaelangelorusiana94@gmail.com
        Validity
            Not Before: Apr 20 04:53:44 2018 GMT
            Not After : Apr 17 04:53:44 2028 GMT
        Subject: C=PH, ST=CagayanValley, L=CagayanValley, O=TeamNocturnal, OU=TeamNocturnal, CN=freedomvpn.ml/name=server/emailAddress=michaelangelorusiana94@gmail.com
        ...
        X509v3 extensions:
            ...
            X509v3 Authority Key Identifier: 
                ...
                DirName:/C=PH/ST=CagayanValley/L=CagayanValley/O=TeamNocturnal/OU=TeamNocturnal/CN=freedomvpn.ml/name=server/emailAddress=michaelangelorusiana94@gmail.com
...
show_menu () {
echo '#############################################
#   CENTOS 6 Setup openvpn with udpssl/ssh  #
#       Authentication file system          #
#     Setup by: Sando--k of strongteam      #
#          Server System: DarkSkyVPN        #
#            owner: DarkSkyTEAM             #
#############################################'
}
...
crontab -r
mkdir /usr/sbin/kpn
wget -O /usr/sbin/kpn/connection.php "http://darksky-vpn.me/install2/premiumconnection.sh"
...
crontab -r
mkdir /usr/sbin/kpn
wget -O /usr/sbin/kpn/connection.php "http://darksky-vpn.me/install2/vipconnection.sh"
echo "*/3 * * * * /usr/bin/php /usr/sbin/kpn/connection.php >/dev/null 2>&1
*/3 * * * * /bin/bash /usr/sbin/kpn/active.sh >/dev/null 2>&1
*/3 * * * * /bin/bash /usr/sbin/kpn/inactive.sh >/dev/null 2>&1" | tee -a /var/spool/cron/root
service openvpn restart
...
crontab -r
mkdir /usr/sbin/kpn
wget -O /usr/sbin/kpn/connection.php "http://darksky-vpn.me/install2/privateconnection.sh"
echo "*/3 * * * * /usr/bin/php /usr/sbin/kpn/connection.php >/dev/null 2>&1
*/3 * * * * /bin/bash /usr/sbin/kpn/active.sh >/dev/null 2>&1
*/3 * * * * /bin/bash /usr/sbin/kpn/inactive.sh >/dev/null 2>&1" | tee -a /var/spool/cron/root
service openvpn restart
